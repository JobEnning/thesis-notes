...
\begin{tcolorbox}[title = Sources, colback = blue!5, colframe = blue]
...
\textbf{Questions}
\begin{enumerate}
    \item When do we use the basic model, and when do we use the model that attaches to the leads?
\end{enumerate}
...
\textbf{Questions}
\begin{enumerate}
    \item What does the simplified model even mean? Modeling the superconductor with a single state? Is there any physical relevance, or is it only for showcasing the formalism?
    \item What is meant with a one-lead model?
    \item The wide-band approximation?
    \item This whole thesis is in equilibrium, but Keldysh formalism is used for non-equilibrium situations. Do we actually use Keldysh because we want to work out of equilibrium, or will we still just look at a steady state solution?
    \item How does the field theory approach make this derivation easier?
    \item What makes implementing the actual Kitaev chain hard (e.g. numerics)?
